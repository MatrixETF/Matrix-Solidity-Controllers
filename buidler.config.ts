...
const PLACE_HOLDER_ADDRESS = "0x0000000";
...
task("deploy-matrix-smart-pool-factory", "deploys a matrix smart pool factory")
  ...
  .setAction(async(taskArgs, { ethers, run }) => {
    ...
    await implementation.init(PLACE_HOLDER_ADDRESS, "IMPL", "IMPL", "1225");

    await factory.init(taskArgs.balancerFactory, implementation.address);
    return factory.address;
});

task("deploy-pool-from-factory", "deploys a matrix smart pool from the factory")
  .addParam("factory")
...
task("deploy-matrix-smart-pool", "deploys a matrix smart pool")
  .setAction(async(taskArgs, { ethers, run }) => {
    const signers = await ethers.getSigners();
...
task("deploy-smart-pool-implementation-complete")
  ...
  .setAction(async(taskArgs, { ethers, run }) => {
    ...
    // Deploy capped pool
    const implementation = await run("deploy-matrix-smart-pool");
...
task("deploy-smart-pool-complete")
  ...
  .setAction(async(taskArgs, { ethers, run }) => {
    ...
    const smartPoolFactoryAddress = await run("deploy-matrix-smart-pool-factory", {balancerFactory: taskArgs.balancerFactory});
...
task("set-join-exit-enabled", "set join exit enabled on a capped pool")
  .addParam("pool")
  .setAction(async(taskArgs, { ethers }) => {
    const signers = await ethers.getSigners();
    const smartpool = Pv2SmartPoolFactory.connect(taskArgs.pool, signers[0]);
    const tx = await smartpool.setJoinExitEnabled(true, {gasLimit: 2000000});
    console.log(`set join exit enabled tx: ${tx.hash}`);
});

task("smart-pool-set-controller", "set controller a capped pool")
  .addParam("pool")
  .addParam("controller")
  .setAction(async(taskArgs, { ethers }) => {
    const signers = await ethers.getSigners();
    const smartpool = Pv2SmartPoolFactory.connect(taskArgs.pool, signers[0]);
    const tx = await smartpool.setController(taskArgs.controller);
    const receipt = await tx.wait(1);
    console.log(`set controller tx: ${tx.hash}`);
});

task("smart-pool-set-swap-fee", "set swap fee a capped pool")
  .addParam("pool")
  .addParam("fee")
  .setAction(async(taskArgs, { ethers }) => {
    const signers = await ethers.getSigners();
    const smartpool = Pv2SmartPoolFactory.connect(taskArgs.pool, signers[0]);
    const tx = await smartpool.setSwapFee(parseEther(taskArgs.fee), {gasLimit: 2000000});
    const receipt = await tx.wait(1);
    console.log(`set swap fee tx: ${tx.hash}`);
  });
...
task("exit-smart-pool")
  .addParam("pool")
  .addParam("amount")
  .setAction(async(taskArgs, { ethers }) => {
    const signers = await ethers.getSigners();
    const smartpool = Pv2SmartPoolFactory.connect(taskArgs.pool, signers[0]);
    const tx = await smartpool.exitPool(parseEther(taskArgs.amount), {gasLimit: 2000000});
    const receipt = await tx.wait(1);
    console.log(`Pool exit tx: ${receipt.transactionHash}`)
  });
...
task("approve-smart-pool")
  ...
  .setAction(async(taskArgs, { ethers }) => {
    ...
    for(const tokenAddress of tokens) {
      ...
      console.log(await token.allowance(account, smartpool.address));
    }
});
